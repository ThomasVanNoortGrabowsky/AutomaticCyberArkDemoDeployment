<#
.SYNOPSIS
  Full pipeline using linked-clone Packer output.
...
if (Test-Path $outputLinked) {
  ...
  Remove-Item $outputLinked -Recurse -Force
}
...
if ($LASTEXITCODE -ne 0) { Write-Error 'VMREST did not respond.'; exit 1 }

# 8) Inventory & choose template
...
for ($i=0; $i -lt $vms.Count; $i++) {
  $n = if ($vms[$i].displayName) { $vms[$i].displayName } else { '(blank)' }
  Write-Host (" [{0}] {1,-20} : {2}" -f $i, $n, $vms[$i].id)
}
$template = $vms | Where-Object { $_.displayName -match 'Win2022' } | Select-Object -First 1
if (-not $template) {
  Write-Warning 'No match; using first entry.'; $template = $vms[0]
}
