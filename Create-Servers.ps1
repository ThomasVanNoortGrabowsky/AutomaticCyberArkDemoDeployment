...
# 2) Install Packer v1.11.2 locally if needed
...
# 6) Strip out all provisioners (only WinRM is baked by the unattended file)
...
Write-Host "Removed all provisioners; Packer will only install & wait for WinRM." -ForegroundColor Green

# 7) Clean old output & build with Packer
...
# Write main.tf with conditional Vault resource
@'
terraform {
  required_providers {
    vmworkstation = {
...
variable "vmrest_user"     { type = string   default = "vmrest" }
variable "vmrest_password" { type = string   default = "Cyberark1" }
variable "vault_image_id"  { type = string   default = "thomas" }
variable "app_image_id"    { type = string   default = "thomas-app" }
variable "vm_processors"   { type = number   default = 2 }
variable "vm_memory"       { type = number   default = 2048 }
variable "vm_path"         { type = string }
variable "create_vault"    { type = bool     default = false }
...
Write-Host "🎉 CyberArk Vault VM deployment complete!" -ForegroundColor Green
